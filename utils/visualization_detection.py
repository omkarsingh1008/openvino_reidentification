...
def draw_detections(frame, detections, show_all_detections=False):
    """Draws detections and labels"""
    for i, obj in enumerate(detections):
        left, top, right, bottom = obj#obj.rect
        label = ""#obj.label
        cv.rectangle(frame, (left, top), (right, bottom),(0,0,0),thickness=3)
        ...
        """id = 1 #int(label.split(' ')[-1]) if isinstance(label, str) else int(label)
...
def visualize_multicam_detections(frames, all_objects, fps='', show_all_detections=True,
    ...
    cv.putText(vis, str(fps)+"omkar", (base_line*2, base_line*3),
